/*
 * Arduino SoftwareSerial API implementation for MCUI by gloomyandy
 *
...
#warning Timer 2 is also used for the Tone generation
constexpr MCUI::IRQNumber software_serial_irq = MCUI::IRQNumber::TIMER2;
auto& software_serial_timer = MCUI::timer2;

#define FORCE_BAUD_RATE 19200
#warning SoftwareSerial baud rate will be forced to 19200 baud
...
{
  if (speed != cur_speed) {
    MCUCore::nvic_disable_irq(software_serial_irq);
    if (speed != 0) {
      software_serial_timer.TCR = MCUI::util::bit_value(1);                      // Put Timer in reset state
      software_serial_timer.PR = 0;
      software_serial_timer.MCR = MCUI::util::bitset_value(0, 1);                // Match on MR0, reset on MR0, interrupts when NVIC enables them
      software_serial_timer.MR0 = (PeripheralClock / (speed * OVERSAMPLE)) - 1;  // Match value (period) to set frequency
      MCUCore::nvic_enable_irq(software_serial_irq);
      software_serial_timer.TCR = MCUI::util::bit_value(0);                      // Start Timer
    }
    cur_speed = speed;
  }
}
...
inline void SoftwareSerial::setTX() {
  ...
  MCUI::gpio_set(_transmitPin, _inverse_logic ? LOW : HIGH);
...
[[gnu::always_inline, gnu::optimize("O3")]] inline void SoftwareSerial::send() {
  if (--tx_tick_cnt <= 0) {
    if (tx_bit_cnt++ < 10 ) {
      ...
      MCUI::gpio_set(_transmitPin, tx_buffer & 1);
      tx_buffer >>= 1;
...
[[gnu::always_inline, gnu::optimize("O3")]] inline void SoftwareSerial::recv() {
  if (--rx_tick_cnt <= 0) {
    uint8_t inbit = MCUI::gpio_get(_receivePin);
    if (rx_bit_cnt == -1) {
...
SoftwareSerial::SoftwareSerial(pin_t receivePin, pin_t transmitPin, bool inverse_logic /* = false */) :
  ...
  MCUI::register_isr( software_serial_irq, [](){
    software_serial_timer.IR = software_serial_timer.IR; //clear interrupt
...
void SoftwareSerial::begin(long speed) {
  ...
  _speed = speed;
  if (!initialised) {
    MCUI::peripheral_power_on(MCUI::PeripheralPowerControl::TIM2);
    MCUCore::nvic_set_priority(software_serial_irq, MCUCore::nvic_encode_priority(0, INTERRUPT_PRIORITY, 0));
    initialised = true;
  }
  if (!_half_duplex) {
    setTX();
