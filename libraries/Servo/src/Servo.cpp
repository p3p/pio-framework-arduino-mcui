...
int8_t Servo::attach(const pin_t pin, const int min, const int max) {
  if (this->servoIndex >= MAX_SERVOS || !MCUI::pwm_attach_pin(pin, DEFAULT_PULSE_WIDTH)) return -1;
...
void Servo::detach() {
  ...
  MCUI::pwm_detach_pin(servo_info[this->servoIndex].Pin.nbr);
}
...
void Servo::writeMicroseconds(int value) {
  ...
  if (channel < MAX_SERVOS) {  // ensure channel is valid
    ...
    MCUI::pwm_attach_pin(servo_info[this->servoIndex].Pin.nbr);
    MCUI::pwm_write_us(servo_info[this->servoIndex].Pin.nbr, value);
  }
}
